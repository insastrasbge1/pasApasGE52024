 ...
 */
package fr.insa.beuvron.pasapas.trie2;
    ...
    public static void triBorne(int[] tab, int min, int max, int[] buf) {
        if (max - min < 3) {
            ...
            Thread premiereMoitie = new Thread(() -> {
                ...
                Utils.joinSansInterrupt(secondQuart);
            ...
            Thread secondeMoitie = new Thread(() -> {
                ...
                Utils.joinSansInterrupt(troisiemeQuart);
                Utils.joinSansInterrupt(quatriemeQuart);
                fusion(tab, m2+1, max, buf);
            });
            premiereMoitie.start();
            secondeMoitie.start();
            Utils.joinSansInterrupt(premiereMoitie);
            Utils.joinSansInterrupt(secondeMoitie);
